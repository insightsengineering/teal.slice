...
DFFilterStates <- R6::R6Class( # nolint
  ...
  public = list(
    ...
    set_filter_state = function(data, state, vars_include = get_supported_filter_varnames(data = data), ...) {
      ...
      logger::log_trace(
        "{ class(self)[1] }$set_filter_state initializing, dataname: { deparse1(private$dataname) }"
      )
      ...
      if (length(excluded_vars) > 0) {
        ...
        logger::log_warn(
          paste(
            ...
            "from { deparse1(private$dataname) }"
          )
        )
      }
      ...
      logger::log_trace(
        "{ class(self)[1] }$set_filter_state initialized, dataname: { deparse1(private$dataname) }"
      )
      NULL
    },
    ...
    srv_add_filter_state = function(id, data, vars_include = get_supported_filter_varnames(data = data), ...) {
      ...
      moduleServer(
        ...
        function(input, output, session) {
          logger::log_trace(
            "DFFilterStates$srv_add_filter_state initializing, dataname: { deparse1(private$dataname) }"
          )
          ...
          logger::log_trace(
            "DFFilterStates$srv_add_filter_state initialized, dataname: { deparse1(private$dataname) }"
          )
          NULL
        }
      )
