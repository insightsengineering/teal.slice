...
#'   AGE = round(runif(100, min = 20, max = 70))
#' )
#'
...
#'     filter_states$ui_add_filter_state("add_filters", x), br(),
...
#'     actionButton("button1", "set SEX == \"F\"", width = "100%"), br(),
#'     actionButton("button2", "set AGE < 30", width = "100%"), br(),
#'     actionButton("button3", "remove SEX", width = "100%"), br(),
#'     actionButton("button4", "clear all filters", width = "100%"), br()
#'   ))
#' )
#'
#' server <- function(input, output, session) {
#'
#'   filter_states$srv_add_filter_state("add_filters", x)
#'   filter_states$server("fs")
#'
#'   output$call <- renderPrint(filter_states$get_call())
#'   output$formatted <- renderText(filter_states$format())
#'
#'   observeEvent(input$button1, {
#'     filter_state <- list(SEX = list(selected = "M"))
#'     filter_states$set_filter_state(data = x, state = filter_state)
#'   })
#'   observeEvent(input$button2, {
#'     filter_state <- list(AGE = list(selected = c(0, 30)))
#'     filter_states$set_filter_state(data = x, state = filter_state)
#'   })
#'   observeEvent(input$button3, filter_states$state_list_remove(1, state_id = "SEX"))
#'   observeEvent(input$button4, filter_states$state_list_empty())
#' }
#'
#' if (interactive()) {
#'   shinyApp(ui, server)
