...
#' x <- c(NA, sample(letters[1:5], size = 999, replace = TRUE))
#' ## use letters[1:10] to obtain a different input widget
#'
#' # initiate filter state
...
#'     textOutput("condition"), br(),
#'     h4("Unformatted state"), # display raw filter state
#'     textOutput("unformatted"), br(),
#'     h4("Formatted state"), # display human readable filter state
#'     textOutput("formatted"), br()
#'   )),
#'   column(4, div(
#'     h4("Programmatic filter control"),
#'     actionButton("button1", "set selected", width = "100%"), br(),
#'     actionButton("button2", "set filter state", width = "100%"), br(),
#'     actionButton("button3", "set keep NA", width = "100%"), br(),
#'     actionButton("button4", "set drop NA", width = "100%"), br(),
#'     actionButton("button5", "deselect all", width = "100%"), br()
#'   ))
#' )
#'
#' server <- function(input, output, session) {
#'   output$condition <- renderPrint(filter_state$get_call())
#'   output$formatted <- renderText(filter_state$format())
#'   output$unformatted <- renderPrint(filter_state$get_state())
#'
#'   # filterstate$server returns a trigger when the close button is pressed
#'   signal_to_toggle <- filter_state$server("fs")
#'   # shinyjs uses it to toggle a ui element
#'   observeEvent(signal_to_toggle(), shinyjs::toggle("outputs"))
#'
#'   # modify filter state programmatically
#'   observeEvent(input$button1, filter_state$set_selected(c("b", "c")))
#'   observeEvent(input$button2, filter_state$set_state(list(selected = c("c", "b"), keep_na = TRUE)))
#'   observeEvent(input$button3, filter_state$set_keep_na(TRUE))
#'   observeEvent(input$button4, filter_state$set_keep_na(FALSE))
#'   observeEvent(input$button5, filter_state$set_selected(c()))
#' }
#'
#' if (interactive()) {
#'   shinyApp(ui, server)
