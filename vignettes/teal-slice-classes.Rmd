...
One `FilteredData` object is initialized on a list containing supported data objects (`data.frame`, `MAE`).
...
- `ui/srv_add` allows adding a `FilterState` for a selected variable. The variables included in the module are the filterable column names of the provided data set. Selecting a variable adds a `FilterState` to the `reactiveVal` stored in the `private$state_list` private field. The specific variation of the created `FilterState` is automatically determined based on the class of the selected column.
