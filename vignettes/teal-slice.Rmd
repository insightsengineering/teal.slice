...
1) top panel showing the number of records (observations) remaining after the filters have been applied
...
For `CDISC` data, filtering records in a parent dataset (i.e. `ADSL`) will also apply the same filter to all child datasets and in child datasets it is only possible to filter columns not found in its parent's dataset. For general relational data the filter panel treats each dataset independently irrespective of relationships between datasets. This can lead to some unintuitive behaviour so be careful filtering general relational data.
The filter panel also supports filtering `MultiAssayExperiment` data.
