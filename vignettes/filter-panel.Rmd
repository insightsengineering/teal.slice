...
`teal.slice` is composed of multiple class objects which main purpose is to provide shiny module
responsible to display and manage filters. On top of the class structure sits `FilteredData` which 
other modules can use to obtain filtered data and filter calls. 
...
`FilteredData` contains one-to-many `FilteredDataset`. While `FilteredData` manages whole filter panel,
`FilteredDataset` is responsible for single dataset filtering. Depending on a variant of
`FilteredDataset` it can contain one or many `FilterStates`. `FilterStates` is a collection of `FilterState` objects. 
`FilterState` is a single filter condition applied on a column (`FilterState`) or on one-to-many variables 
(`FilterStateExpr`). Each `FilterState` objects contain one `teal_slice` object
which is a `reactiveValuse` and stores all necessary information to define a filter. `teal_slice` object
unlike other classes it has no methods nor shiny modules, it's only purpose is to store filter information.
...
- `ui/srv_add` allow to add `FilterState` for selected variable. Variables included
