...
Constructor of `FilterState` has `extract_type` argument which impacts returned call. `extract_type`
...
- `ReactiveQueue` is a container which stores multiple condition calls. `ReactiveQueue` objects are
  kept in a list where the names of elements are set after argument names.
