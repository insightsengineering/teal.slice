...
testthat::test_that("set_state overwrites fields included in the input only", {
  filter_state <- LogicalFilterState$new(
    x = c(TRUE, FALSE, NA), x_reactive = reactive(NULL), varname = "test", dataname = "data")
  filter_state$set_state(selected = FALSE, keep_na = TRUE, varname = "test", dataname = "data")
  testthat::expect_no_error(filter_state$set_state(selected = TRUE, varname = "test", dataname = "data"))
...
testthat::test_that("is_any_filtered returns TRUE when enabled", {
  ...
  fs$set_state(selected = TRUE, keep_na = TRUE, varname = "x", dataname = "data")
  # fs$disable()
  # fs$enable()
