...
testthat::test_that("get_call returns a condition true for the object in the selected range", {
  filter_state <- DatetimeFilterState$new(posixct, x_reactive = reactive(NULL), varname = "variable", dataname = "data")
  filter_state$set_state(filter_var(selected = posixct[2:3], varname = "variable", dataname = "data"))
...
testthat::test_that("get_call returns a condition evaluating to TRUE for NA values after set_keep_na(TRUE)", {
  ...
  filter_state <- DatetimeFilterState$new(
    variable, x_reactive = reactive(NULL), varname = "variable", dataname = "data")
...
testthat::test_that("DatetimeFilterState echoes the timezone of the ISO object passed to the constructor", {
  ...
  filter_state <- DatetimeFilterState$new(
    objects, x_reactive = reactive(NULL), varname = "objects", dataname = "data")
  testthat::expect_equal(
    shiny::isolate(filter_state$get_call()),
    quote(
...
testthat::test_that(
  ...
  testthat::expect_warning(
    filter_state$set_state(
      filter_var(varname = "objects", dataname = "data", selected = c(objects[1] - 1, objects[1]))
    ),
    "outside of the range"
  )
  testthat::expect_warning(
    filter_state$set_state(
      filter_var(varname = "objects", dataname = "data", selected = c(objects[2], objects[2] + 1))
    ),
    "outside of the range"
  )
  testthat::expect_warning(
    filter_state$set_state(
      filter_var(varname = "objects", dataname = "data", selected = c(objects[1] - 1, objects[2] + 1))
    ),
    "outside of the range"
  )
})
...
testthat::test_that("set_state: selected throws when the selected range is completely outside of the possible range", {
  ...
  testthat::expect_warning(
    filter_state$set_state(
      filter_var(selected = c(objects[2] + 1, objects[2] + 2), varname = "objects", dataname = "data")
    ),
...
testthat::test_that(
  ...
  testthat::expect_equal(shiny::isolate(filter_state$get_state()$selected), c(objects[1], objects[1]))
  suppressWarnings(filter_state$set_state(
    filter_var(varname = "objects", dataname = "data", selected = c(objects[2], objects[2] + 1))
  ))
  testthat::expect_equal(shiny::isolate(filter_state$get_state()$selected), c(objects[2], objects[2]))
...
testthat::test_that("set_state: selected throws when the value type cannot be interpreted as POSIX", {
  ...
  testthat::expect_error(
    filter_state$set_state(filter_var(selected = c("a", "b"), varname = "objects", dataname = "data")),
...
testthat::test_that("set_state sets values of selected and keep_na as provided in the list", {
  ...
  testthat::expect_identical(shiny::isolate(filter_state$get_state()$selected), c(objects[2], objects[3]))
...
testthat::test_that("set_state overwrites fields included in the input only", {
  ...
  testthat::expect_identical(shiny::isolate(filter_state$get_state()$selected), c(objects[3], objects[4]))
  testthat::expect_true(shiny::isolate(filter_state$get_state()$keep_na))
})
...
testthat::test_that(
  ...
  code = {
    ...
    shiny::isolate(filter_state$set_state(filter_var(keep_na = FALSE, varname = "x", dataname = "data")))
    testthat::expect_true(
      shiny::isolate(filter_state$is_any_filtered())
    )

    shiny::isolate(filter_state$set_state(filter_var(keep_na = TRUE, varname = "x", dataname = "data")))
...
testthat::test_that("is_any_filtered returns TRUE when enabled", {
  ...
  fs$set_state(filter_var(selected = datetime_seq[1:2], keep_na = TRUE, varname = "x", dataname = "data"))
  fs$disable()
  fs$enable()
