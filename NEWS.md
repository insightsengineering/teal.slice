...
* Parameters `input_dataname` and `output_dataname` of `FilterStates` are now combine into one character parameter `dataname`.
* Parameter `varname` of `FilterState` is not a character.
